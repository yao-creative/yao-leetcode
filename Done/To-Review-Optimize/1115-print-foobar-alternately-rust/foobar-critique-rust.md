...
pub struct FooBar {
    n: i32,
    foo_sem: Arc<Semaphore>,
    bar_sem: Arc<Semaphore>,
}
...
impl FooBar {
    pub fn new(n: i32) -> Self {
        Self {
            n,
            foo_sem: Arc::new(Semaphore::new(1)),
            bar_sem: Arc::new(Semaphore::new(0)),
        }
    }
...
fn main() {
    ...
    let f1 = Arc::clone(&foobar);
    let f2 = Arc::clone(&foobar);

    let t1 = thread::spawn(move || f1.foo(|| print!("foo")));
    let t2 = thread::spawn(move || f2.bar(|| print!("bar")));
